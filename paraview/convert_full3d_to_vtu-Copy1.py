...
# revrs = f3d.rs[::-1]
# rs = f3d.rs - np.trapz(revrs**2,revrs)/(nr)
rs = f3d.rs
thetas = f3d.thetas
...
for f in files:
  ...
  for index, vals in f3d.vals.iteritems():
    ...
    if index == '0501':
        vals = np.reshape(vals,(nphi,ntheta,nr), order = 'F')
        for i in range(nr):
            mean = np.mean(vals[:,:,i])
            vals[:,:,i] = vals[:,:,i] - mean
        vals = np.reshape(vals,(nphi*ntheta*nr), order = 'F')
  ...
  gridwriter.SetFileName(os.path.join("vtu2_files", "full3d_"+f3d.basefilename+".vtu"))
